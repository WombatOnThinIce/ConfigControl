...
class ConfigItem:
    def __init__(self, item_type: str, serial: int, name: str, ci_id: str, creation_notes: str) -> None:
        if item_type not in CI_TYPES:
        ...
        self.log = [create_log_entry(f"CI_CREATED - type {self.item_type}>> " + creation_notes)]
        self.versions = [Version(1, f"CI_CREATED - type {self.item_type}>> " + creation_notes)]

    def add_new_version(self, notes: str) -> Version:
        new_serial = len(self.versions) + 1
        new_version = Version(new_serial, notes)
        self.versions.append(new_version)
        self.log.append(create_log_entry(f"VERSION_CREATED V{new_version.version}>> " + notes))
